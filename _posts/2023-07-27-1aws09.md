...
- Lambda URL은 Lambda 코드를 API 형태로 쉽게 사용할 수 있도록 도와준다. 아래와 같이 함수가 return하는 JSON 값의 headers > Content-Type 부분 설정을 통해 body에 담긴 string을 읽어 HTML로 보여줄 수도 있다.
  ...
  ```
- 해당 기능을 이용한다면 Lambda 함수 실행시 Python의 boto3 라이브러리를 통해 EC2 인스턴스들의 목록을 받아와 HTML 코드 구성 후 정적 웹사이트를 띄우고, UI상 버튼의 on click 기능을 통해 지정된 Python 함수를 실행시켜 인스턴스의 시작,중지 명령을 내릴 수 있게된다.
- 전체적인 실행 순서는 다음과 같다.
    ...
    4. 사용자의 start/stop 명령 있을시 선택한 EC2 이름을 POST response로 Lambda에 전송
    5. start인지 stop인지에 따라 그에 맞게 Boto3를 통해 instance에 명령 전달.
